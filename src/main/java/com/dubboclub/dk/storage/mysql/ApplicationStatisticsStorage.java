...
public class ApplicationStatisticsStorage extends AbstractApplicationStatisticsStorage {
    ...
    public ApplicationStatisticsStorage(ApplicationMapper applicationMapper, StatisticsMapper statisticsMapper,
        DataSource dataSource, TransactionTemplate transactionTemplate, String application, int type,
        Long dataCleanInterval, Long dataSaveMaxTime) {
        this(applicationMapper, statisticsMapper, dataSource, transactionTemplate, application, type, dataCleanInterval,
            dataSaveMaxTime, false);
    }

    public ApplicationStatisticsStorage(ApplicationMapper applicationMapper, StatisticsMapper statisticsMapper,
        DataSource dataSource, TransactionTemplate transactionTemplate, String application, int type,
        Long dataCleanInterval, Long dataSaveMaxTime, boolean needCreateTable) {
        super(application, dataCleanInterval, dataSaveMaxTime);
    ...
    protected void clean() {
        this.transactionTemplate.execute(new TransactionCallback<Boolean>() {
            ...
            public Boolean doInTransaction(TransactionStatus status) {
                try {
                    // (1) 先判断统计表是否存在
                    boolean tableExist = (1 == statisticsMapper.queryStatisticsTableExist(application) ? true : false);
                    if (tableExist) {
                        // (2) 统计表删除时间早于now()-dataSaveMaxTime的记录
                        Long cleanTime = System.currentTimeMillis() - getDataSaveMaxTime() * 1000;
                        int deletedCount = statisticsMapper.deleteDataSince(application, cleanTime);
                        // (3)如果清理后表数据为空,表示这个应用已经很长时间没有用, 我们认定应用作废,删表清数据
                        List<ServiceInfo> serviceInfoList = statisticsMapper.queryServiceByApp(application);
                        if (CollectionUtils.isEmpty(serviceInfoList)) {
                            statisticsMapper.dropTable(application);
                            applicationMapper.deleteApplication(application);
                            MysqlStatisticsStorage.APPLICATION_STORAGES.remove(application);
                        }
                    } else {
                        // 清理application表, 删除对应记录
                        applicationMapper.deleteApplication(application);
                        MysqlStatisticsStorage.APPLICATION_STORAGES.remove(application);
                    }
                    return true;
