...
public class MysqlStatisticsStorage implements StatisticsStorage, InitializingBean {
    ...
    public static final ConcurrentHashMap<String, ApplicationStatisticsStorage> APPLICATION_STORAGES =
        new ConcurrentHashMap<String, ApplicationStatisticsStorage>();

    private Long dataCleanInterval = 1800L; // 默认30分钟

    private Long dataSaveMaxTime = 2592000L; // 默认30天
    ...
    public void storeStatistics(Statistics statistics) {
        if (!APPLICATION_STORAGES.containsKey(statistics.getApplication().toLowerCase())) {
            ApplicationStatisticsStorage applicationStatisticsStorage =
                new ApplicationStatisticsStorage(applicationMapper, statisticsMapper, dataSource, transactionTemplate,
                    statistics.getApplication(), statistics.getType() == Statistics.ApplicationType.CONSUMER ? 0 : 1,
                    dataCleanInterval, dataSaveMaxTime, true);
    ...
    public Collection<MethodMonitorOverview> queryMethodMonitorOverview(String application, String serviceInterface,
        ...
        for (String method : methods) {
            ...
            Long elapsed = statisticsMapper.queryMethodMaxItemByServiceForLong("elapsed", application, serviceInterface,
            ...
            Long input = statisticsMapper.queryMethodMaxItemByServiceForLong("input", application, serviceInterface,
                method, startTime, endTime);
            Double kbps = statisticsMapper.queryMethodMaxItemByServiceForDouble("kbps", application, serviceInterface,
                method, startTime, endTime);
            Long output = statisticsMapper.queryMethodMaxItemByServiceForLong("output", application, serviceInterface,
                method, startTime, endTime);
    ...
    public Collection<ApplicationInfo> queryApplications() {
        ...
        for (ApplicationInfo applicationInfo : applicationInfos) {
            ApplicationStatisticsStorage applicationStatisticsStorage =
                APPLICATION_STORAGES.get(applicationInfo.getApplicationName());
    ...
    public StatisticsOverview queryApplicationOverview(String application, long start, long end) {
        ...
        List<Statistics> statisticses =
            statisticsMapper.queryApplicationOverview(application, "concurrent", start, end);
    ...
    public StatisticsOverview queryServiceOverview(String application, String service, long start, long end) {
        StatisticsOverview statisticsOverview = new StatisticsOverview();
        List<Statistics> statisticses =
            statisticsMapper.queryServiceOverview(application, service, "concurrent", start, end);
        fillConcurrentItem(statisticses, statisticsOverview);
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        for (ApplicationInfo app : apps) {
            ApplicationStatisticsStorage applicationStatisticsStorage =
                new ApplicationStatisticsStorage(applicationMapper, statisticsMapper, dataSource, transactionTemplate,
                    app.getApplicationName(), app.getApplicationType(), dataCleanInterval, dataSaveMaxTime);
    ...
    public void setDataCleanInterval(Long dataCleanInterval) {
    ...
    public void setDataSaveMaxTime(Long dataSaveMaxTime) {
