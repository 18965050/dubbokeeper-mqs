...
public class DubboKeeperMonitorService implements MonitorService {
    ...
    public void collect(URL statisticsURL) {
        ...
        if (statistics.getInput() != 0 && statistics.getElapsed() != 0) {
            // kbps=tps*平均每次传输的数据量
            kbps = kbps.multiply(BigDecimal.valueOf(statistics.getInput()).divide(BigDecimal.valueOf(1024), 2,
                BigDecimal.ROUND_HALF_DOWN));
        } else if (statistics.getElapsed() != 0) {
            kbps = kbps.multiply(BigDecimal.valueOf(statistics.getOutput()).divide(BigDecimal.valueOf(1024), 2,
                BigDecimal.ROUND_HALF_DOWN));
        }
